...
function showToast(msg, type = "success") {
    ...
    toast.className = `fixed bottom-5 right-5 z-50 px-5 py-3 rounded-xl shadow-lg text-white font-medium flex items-center gap-2 ${type === 'error' ? 'bg-red-500' : 'bg-blue-500'}`;
...
async function renderDashboard() {
    ...
    if (currentUser.role === 'admin') {
        ...
        menus.forEach(menu => {
            ...
            btn.className = "px-4 py-2 rounded-full bg-white/30 hover:bg-white/50 text-blue-800 font-medium transition flex items-center gap-2 backdrop-blur-sm";
...
async function renderAdminMembers(container) {
    ...
    container.innerHTML = `<div class="flex justify-between items-center mb-4"><h2 class="text-2xl font-bold text-blue-700"><i class="fas fa-users text-blue-500"></i> Manajemen Anggota</h2><button id="addMemberBtn" class="bg-blue-500 text-white px-4 py-2 rounded-xl shadow"><i class="fas fa-user-plus"></i> Tambah Anggota</button></div>
    <div class="overflow-x-auto"><table class="w-full table-modern bg-white/70 rounded-xl"><thead class="bg-blue-50"><tr><th>Nama</th><th>Kelas</th><th>Telepon</th><th>Tgl Daftar</th><th>Username</th><th>Password</th><th>Status Peminjaman</th><th>Aksi</th></tr></thead><tbody id="membersTableBody"></tbody></table></div>`;
    const renderTable = () => {
        document.getElementById("membersTableBody").innerHTML = members.map(m => {
            ...
            return `<tr id="${rowId}">
                ...
                <td class="font-medium"><button data-id="${m.id}" class="editMemberBtn text-blue-600 mr-2"><i class="fas fa-edit"></i></button><button data-id="${m.id}" class="deleteMemberBtn text-red-500"><i class="fas fa-trash"></i></button></td>
              </tr>`;
        }).join('');
...
async function renderAdminLoans(container) {
    ...
    const renderTable = () => {
        document.getElementById("loansTableBody").innerHTML = loans.map(l => {
            ...
            return `<tr><td class="text-2xl">${l.bookCover}</td><td class="font-medium">${l.memberName}</td><td class="font-medium">${l.bookTitle}</td><td class="font-medium">${formatDate(l.loanDate)}</td><td class="font-medium">${formatDate(l.dueDate)}</td><td class="font-medium"><span class="px-2 py-1 rounded-full text-xs ${statusClass}">${statusText}</span></td><td class="font-medium">${l.status === 'borrowed' ? `<button data-id="${l.id}" class="extendLoanBtn bg-blue-400 text-white px-2 py-1 rounded mr-1"><i class="fas fa-calendar-plus"></i> Perpanjang</button><button data-id="${l.id}" class="returnLoanBtn bg-blue-600 text-white px-2 py-1 rounded mr-1">Kembalikan</button><button data-id="${l.id}" class="deleteLoanBtn bg-red-500 text-white px-2 py-1 rounded">Hapus</button>` : `<button data-id="${l.id}" class="deleteLoanBtn bg-red-500 text-white px-2 py-1 rounded">Hapus</button>`}</td></tr>`;
        }).join('');
...
async function renderAdminReturns(container) {
    ...
    container.innerHTML = `<h2 class="text-2xl font-bold mb-3 text-blue-700"><i class="fas fa-undo-alt text-blue-500"></i> Data Pengembalian</h2><div class="overflow-x-auto"><table class="w-full table-modern bg-white/70 rounded-xl"><thead class="bg-blue-50"><tr><th>Peminjam</th><th>Buku</th><th>Tgl Pinjam</th><th>Jatuh Tempo</th><th>Tgl Kembali</th><th>Denda</th><th>Status Bayar</th></tr></thead><tbody>${returns.map(r => {
        const fineColor = r.lateFee > 0 ? 'text-red-600' : 'text-blue-500';
...
async function renderAdminFines(container) {
    ...
    container.innerHTML = `<h2 class="text-2xl font-bold mb-3 text-blue-700"><i class="fas fa-coins text-blue-500"></i> Denda Belum Lunas</h2><div class="overflow-x-auto"><table class="w-full table-modern bg-white/70 rounded-xl"><thead class="bg-blue-50"><tr><th>Peminjam</th><th>Buku</th><th>Jatuh Tempo</th><th>Tgl Kembali</th><th>Denda</th><th>Bukti</th><th>Aksi</th></tr></thead><tbody>${unpaid.map(r => {
        let proofHtml = '-';
...
async function renderUserBorrow(container) {
    ...
    let books = await loadBooksForce();
    container.innerHTML = `<h2 class="text-2xl font-bold mb-3 text-blue-700"><i class="fas fa-book-open text-blue-500"></i> Peminjaman Buku</h2><input id="searchBookUser" placeholder="Cari judul..." class="border p-2 rounded w-full md:w-64 mb-4"><div id="bookListUser" class="grid md:grid-cols-2 lg:grid-cols-3 gap-4"></div>`;
    const render = (bk) => {
        document.getElementById("bookListUser").innerHTML = bk.map(b => `<div class="bg-white/80 p-4 rounded-2xl shadow text-center"><div class="text-5xl">${b.cover}</div><h3 class="font-bold">${b.title}</h3><p>${b.publisher}</p><p>Tersedia: ${b.availableStock}</p>${b.availableStock > 0 ? `<button data-id="${b.id}" class="borrowUserBtn mt-3 w-full bg-blue-500 text-white py-2 rounded-xl">Pinjam</button>` : '<button disabled class="mt-3 w-full bg-gray-300 py-2 rounded-xl">Stok Habis</button>'}</div>`).join('');
...
async function renderUserLoans(container) {
    ...
    container.innerHTML = `<h2 class="text-2xl font-bold mb-3 text-blue-700">Peminjaman Saya</h2><div class="overflow-x-auto"><table class="w-full table-modern bg-white/70 rounded-xl"><thead class="bg-blue-50"><tr><th>Cover</th><th>Buku</th><th>Tgl Pinjam</th><th>Jatuh Tempo</th><th>Status</th></tr></thead><tbody>${myLoans.map(l => {
        const overdue = l.status === 'borrowed' && isOverdue(l.dueDate);
...
async function renderUserReturn(container) {
    ...
    container.innerHTML = `<h2 class="text-2xl font-bold mb-3 text-blue-700">Kembalikan Buku</h2><div class="grid gap-3">${active.length === 0 ? '<div class="bg-white/50 p-4 rounded-xl">Tidak ada buku yang dipinjam</div>' : active.map(l => `<div class="bg-white/80 p-4 rounded-xl flex justify-between items-center"><div><div class="text-2xl inline-block mr-2">${l.bookCover}</div><b>${l.bookTitle}</b><br><small>Jatuh tempo: ${formatDate(l.dueDate)}</small></div><button data-id="${l.id}" class="returnUserBtn bg-blue-500 text-white px-4 py-2 rounded-xl">Kembalikan</button></div>`).join('')}</div>`;
...
async function renderUserFines(container) {
    ...
    container.innerHTML = `<h2 class="text-2xl font-bold mb-3 text-blue-700"><i class="fas fa-coins text-blue-500"></i> Denda Saya</h2><div class="overflow-x-auto"><table class="w-full table-modern bg-white/70 rounded-xl"><thead class="bg-blue-50"><tr><th>Buku</th><th>Jatuh Tempo</th><th>Tgl Kembali</th><th>Denda</th><th>Status</th><th>Aksi</th></tr></thead><tbody>${myFines.map(r => {
...
document.getElementById("loginTabBtn").onclick = () => {
    ...
    document.getElementById("loginTabBtn").classList.add("bg-white","text-blue-600");
    document.getElementById("registerTabBtn").classList.remove("bg-white","text-blue-600");
};
document.getElementById("registerTabBtn").onclick = () => {
    document.getElementById("registerForm").classList.remove("hidden");
    document.getElementById("loginForm").classList.add("hidden");
    document.getElementById("registerTabBtn").classList.add("bg-white","text-blue-600");
    document.getElementById("loginTabBtn").classList.remove("bg-white","text-blue-600");
};
